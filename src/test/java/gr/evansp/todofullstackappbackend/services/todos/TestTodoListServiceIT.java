...
class TestTodoListServiceIT extends BaseITTest {
  ...
  void testStore_invalidInput() {
    ...
    ConstraintViolationException e =
        assertThrows(ConstraintViolationException.class, () -> service.store(todoList));
  ...
  void testStore_unauthorized() {
    ...
    UnauthorizedException e =
        assertThrows(UnauthorizedException.class, () -> service.store(todoList));
    assertEquals(UnauthorizedException.UNAUTHORIZED, e.getMessage());
  }
  ...
  void testUpdate_invalidInput() {
    ...
    todoList.setUserId(SUB);
    ConstraintViolationException e =
        assertThrows(ConstraintViolationException.class, () -> service.update(todoList));
    assertTrue(e.getMessage().contains(VALIDATION_MESSAGES.getString("title.empty")));
  }
  ...
  @Test
  void testDelete_ok() {
    TodoList list1 = service.store(new TodoList(SUB, "sample list"));
    service.delete(list1.getTodoListId());
    assertFalse(service.getAll().contains(list1));
  }
}
